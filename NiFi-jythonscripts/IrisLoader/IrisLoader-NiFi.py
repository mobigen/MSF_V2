...
if originalFlowFile != None:
	...
	try:
		...
		# Transfer
		flowFile = session.create(originalFlowFile) #OriginalFlowFile Attributes 상속?
		#flowFile = session.putAttribute(originalFlowFile, 'filename', os.path.basename(dat_path))
		flowFile = session.write(flowFile, WriteCallback(json.dumps(info, indent=4))) #FlowFile에 Load 관련 Attributes 입력 
