...
class Filetime() :
			...
			for index in range(len(_FilePath)) :
				try:
					...
					for FindStr in _FindString[index].split('|') :
						cmd = 'stat --printf="Modify %%y\n" %s' % (_FilePath[index])
						cmdList.append(['', cmd])
				except:
					#No _FindString[index] 
					for FinsStr in _FindString[0].split('|') :
						cmd = 'stat --printf="Modify %%y\n" %s' % (_FilePath[index])
						cmdList.append(['', cmd])
				li = []
	...
	def run(self) :
		__LOG__.Trace("[Collect]FILE TIME START_______________________")		
		ResultDict = {}
		try :
			for key in self.htConfig.keys() :
		...
		__LOG__.Trace("[Collect]FILE TIME END_______________________")
