...
# V2.0    | 070628 : jjinylee : buffer 모드, SendMessage -> SendMessageTimeout
# V2.1    | 070630 : jjinylee : log 를 client 와  관련된 log 는 INFO 로 남겨줌
# V2.2    | 070705 : jjinylee : SendMessageTimeout 의 timeout 을 5sec
# V2.3    | 070706 : jjinylee : putNoti - dereg 할때 connectorList 의 조작이 일어나므로, index 로 접근
# V3.0    | 070706 : jjinylee : health check
# V3.1    | 070928 : jjinylee : SendMessageTimeout fail 시 , retry Argument 처리 - 1번 retry
# V3.2	  | 071121 : jjinylee : Client -> BUS 로 NTI/STI 보낼때 strip('\n' ) 하던걸 안하게 수정 ( gui 에서 보내는 대로 by pass )
#---------------------------------------------------------------
...
g_RETRY_SENDFLAG= False		# if Client send SendMessage, Timeout fail, Retry Flag
		...
		try :		
			...
			#{
				...
				if( connectorLen <  totalCon  ) :
					__LOG__.Trace( "%-10s| retry SEND To client END ------------------" %('INFO' ), logging.INFO )
					break
...
def main():
	...
	optList, args = getopt.getopt(sys.argv[1:], optStr+'t:r', ['level=', 'output-file='] )
...
if __name__ == "__main__":
	...
	try :
		optStr = sfp.getOptStr()
		optList, args = getopt.getopt(sys.argv[1:], optStr+'t:r', ['level=', 'output-file='] )	# 07.09.28 : 'r' 추가
		...
		print 'examp : %s -t100 -r 10.10.10.10 1234 0,*,* WinAdapter' % (sys.argv[0])

		sfp.prnOpt()
		print '        -t       	: SendMessage Timeout , second, default=180sec'
		print '        -r       	: if fail SendMessage , retry 1 time'
