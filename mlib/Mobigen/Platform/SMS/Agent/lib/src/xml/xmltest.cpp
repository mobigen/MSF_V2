...
*/
	...
	if ( !loadOkay )
	{
		printf( "Could not load test file 'agentconf.xml'. Error='%s'. Exiting.\n", doc.ErrorDesc() );
		exit( 1 );
	}
	...
	TiXmlAttribute *att = itemElement->FirstAttribute();
	printf("************* Attribure Name  : %s",att->Name());
	printf("************* Attribure Value : %s",att->Value());
