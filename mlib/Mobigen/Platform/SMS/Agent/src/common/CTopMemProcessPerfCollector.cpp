...
CTopMemoryProcessPerfCollector::CTopMemoryProcessPerfCollector()
{
	m_list = NULL;
}
...
void CTopMemoryProcessPerfCollector::collect()
{
	char *msg=NULL;
	...
	if(	(m_list = scCoreViewPSInfo(m_coreview,SC_SUM_BY_DEFAULT,SC_SORT_BY_PMEM))==NULL ) {
...
void CTopMemoryProcessPerfCollector::makeMessage()
{
	int i=0, at=0, topn=-1;
	...
	sprintf(buf, "Instance%cCommand%cCpuTime%cMemoryUsage\n", tab, tab, tab);
	m_msgfmt.setTitle(buf);

	if(!instQ->isEmpty()){
		char *instname = (char *)instQ->frontNode()->d;
		if(strncmp(instname, "memory", 6)==0){
