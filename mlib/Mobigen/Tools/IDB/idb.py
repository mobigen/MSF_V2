		...
		try:
			while pipe.is_open() and _raiseEx.ex is None:
				...
				try:
					...
					if (code and not multiline_flag) or (multiline_flag and line.strip() == '') :
						multiline_flag = False
...
if __name__ == '__main__':
	if len(sys.argv) != 2:
		print >> sys.stderr, 'Error : Must provide process id to debug'
		print >> sys.stderr, 'Usage : %s pid' % os.path.basename(sys.argv[0])
		print >> sys.stderr, 'Exam  : %s %s' % (os.path.basename(sys.argv[0]), os.getpid())
	else:
		pid = int(sys.argv[1])
		debug_process(pid)
		print ""
