...
class SSHTailer:
    def __init__(self, sHost, sPort, sUser, sPass,
                 sRdir, sPatn, sBase, sLdir, sRidx, sSect):#, sIdir, sInam):
        self.bLoop = True
    ...
    def check(self):
        if not self.isexistname():
            ...
            # self.sName이 getnextnames 할 때의 기준 index가 됨
            self.sName = sName
            self.iLine = 1
    ...
    def connect(self):
        ...
        __LOG__.Trace("Conn: %s/%s %s/%s" % \
                (self.sHost, self.sPort, self.sUser, self.sPass))
    ...
    def getnames(self):
        '''
            RDIR 디렉토리에서 PATN 패턴의 파일을 검색후 정렬하여 리스트 형태로 리턴
        '''
        Names = []
...
class Worker:
    def __init__(self, sSect, sConf):
        ...
        try:
            signal.signal(signal.SIGHUP , self.sign)
        except:
            pass
        try:
            signal.signal(signal.SIGPIPE, self.sign)
        except:
            pass
    ...
    def work(self):
        ...
        while self.bLoop:
            # check dictionary
            try:
                self.oData = SSHTailer(self.sHost, self.sPort, self.sUser,
                                       self.sPass, self.sRdir, self.sPatn,
                                       self.sBase, self.sLdir, self.sRidx, self.sSect)
                break
        ...
        while self.bLoop:
            try:
                ...
                try:
                    self.oData = SSHTailer(self.sHost, self.sPort, self.sUser,
                                           self.sPass, self.sRdir, self.sPatn,
                                           self.sBase, self.sLdir, self.sRidx,
                                           self.sSect)
                except paramiko.ssh_exception.AuthenticationException:
                    self.sPass = next(self.pwd_iter)
                except:
                ...
                try:
                    ...
                    if oLine:
                        for sline in oLine:
                            ...
                            if sline:
                                #index 저장
                                self.oData.oIndx.save(self.oData.sName,
                                                      self.oData.iLine)
                                __LOG__.Trace("Save: Indx ( %s, %d )" % \
                                        (self.oData.fileinfo()))
