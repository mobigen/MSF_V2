...
class IrisSplitter():
    ...
    def GetConfig(self):
        try:
            ...
            #개별 설정
            if self.PARSER.has_option(self.SECTION, 'SAVE_PATH'):
                self.SAVE_PATH = self.PARSER.get(self.SECTION, 'SAVE_PATH')
            if self.PARSER.has_option(self.SECTION, 'SAVE_SEPARATE'):
                self.SAVE_SEPARATE = self.PARSER.get(self.SECTION, 'SAVE_SEPARATE')
            if self.PARSER.has_option(self.SECTION, 'FILE_SEPARATE'):
                self.FILE_SEPARATE = self.PARSER.get(self.SECTION, 'FILE_SEPARATE')
            if self.PARSER.has_option(self.SECTION, 'STDIN_STRING'):
                self.STDIN_STRING = self.PARSER.get(self.SECTION, 'STDIN_STRING')
    ...
    def run(self):
        ...
        while G_SHUTDOWN:
            try:
                ...
                FilePath = stdin[len(self.STDIN_STRING):].strip()
                ...
                with open(FilePath, 'r') as fd:
                    for line in fd:
                        row = line.strip().split(self.FILE_SEPARATE)
                        ...
                        if self.PARTITION_RANGE == 1:
                            ...
                            Partition[12] = str(int(Partition[12]) - \
                                                int(Partition[12]) % 5)
                        ...
                        FileName = '%s_%s_%s' % (Key,
                                                 Partition,
                                                 os.path.basename(FilePath))

                        SaveFilePath = os.path.join(self.SAVE_PATH,FileName)

                        row_w = self.SAVE_SEPARATE.join(str(v) for v in row)
                        try:
                            diFile[SaveFilePath].write(row_w + '\n')
                        except:
