...
class MakeIrisLoaderFile():
	...
	def GetConfig(self):
		try :
            ...
            if self.PARSER.has_option(self.SECTION, 'SPLIT_CNT'):
                self.SPLIT_CNT = self.PARSER.getint(self.SECTION, 'SPLIT_CNT')
            ...
            if self.PARSER.has_option(self.SECTION, 'VALUE_START_INDEX'):
                vsidx = self.PARSER.getint(self.SECTION, 'VALUE_START_INDEX')
            if self.PARSER.has_option(self.SECTION, 'VALUE_END_INDEX'):
                veidx = self.PARSER.getint(self.SECTION, 'VALUE_END_INDEX')
	...
	def run(self):
		...
		if len(self.STARTDATE) == 0:
            pd = dt.datetime.strptime((dt.datetime.now().strftime('%Y%m%d%H%M00')),
                    '%Y%m%d%H%M00')
		else:
            pd = dt.datetime.strptime(self.STARTDATE,'%Y%m%d%H%M%S')
		...
		while G_SHUTDOWN:
			try :
				...
				nd =  dt.datetime.now()
				#파티션 Delay 확인
				if pd + dt.timedelta(seconds=self.DELAY_SECOND) > nd:
					__LOG__.Trace('Delay Date[%s] > Now[%s]' % \
                            (pd + dt.timedelta(seconds=self.DELAY_SECOND), nd))
					time.sleep(1)
					continue
				...
				pd += dt.timedelta(seconds=self.GAP_SECOND)
